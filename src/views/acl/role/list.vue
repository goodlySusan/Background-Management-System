    ...
    <div style="margin-top:20px">
        ...
        <el-table border stripe v-loading="listLoading" :data="roles"
            ...
            <el-table-column label="操作" >
                <template slot-scope="{row}">
                    <el-button size="mini" type="success" icon="el-icon-setting" title="分配权限"
                        @click="$router.push(`/acl/role/auth/${row.id}?roleName=${row.roleName}`)"></el-button>
                    <el-button size="mini" type="warning" icon="el-icon-check" title="确定" @click="updateRole(row)"
                        v-if="row.edit"></el-button>
                    <el-button size="mini" type="warning" icon="el-icon-edit" title="修改角色" @click="row.edit = true"
                        v-if="!row.edit"></el-button>
